...
| closedLostInPeriod (canonical Lost Date in window) |   670 |
| createdAndLostInPeriod                             |   286 |
| olderCohortClosedLostInPeriod                      |   384 |
| undatedCohortClosedLostInPeriod                    |     0 |

Closure date evidence: 428 Opportunities use `lost_verification_date`, 189 Opportunities use the
documented `date_last_stage_update` fallback, and 53 Leads use `date_closed`. The date-source split
sums exactly to 670 and is now returned by `/api/lost`.
...
Duplicate is 69 / 286 = 24.13%, not 64%. Independent identity evidence (provider lead id or either
normalised phone field) supports 53 of the 69 classifications across 35,525 unique CRM records; 16
remain explicitly marked for manual review. Identity support is evidence, not an automatic rewrite of
the Odoo reason.

The live registration audit also found two Leads which Odoo marks `LOST` but which have no structured
`lost_reason_id` (CRM ids 66896 and 79431). They stay outside Registered Lost totals and appear in a
separate actionable queue. It also found 101 active Opportunities carrying a stale Lost Reason while
not being in the Lost stage; they remain open and are shown as a data-quality warning, never as Lost.

## 4. Figures that cannot yet be independently verified

1. **CRM cohort drift.** The 19:40 UTC link-graph refresh read 1,699 records created in the window (915 with a Meta lead ID).
...
2. The new engine's after-values must be confirmed after deploy via
   `/api/acquisition/closed-loop?from=2026-09-01&to=2026-09-15` (and `&platform=meta|snapchat|tiktok|google`).
3. Invoice-date revenue is reported on the stored `$ Sales` basis only.

Resolved 2026-09-16: Odoo serialises an empty date as boolean `false`. The direct CRM normaliser had
converted that to the truthy string `"false"`, which blocked the contract's Opportunity fallback from
`lost_verification_date` to `date_last_stage_update`, while a later parser independently fell back to a
legacy `date_closed` column. That produced two contradictory closure totals (481 and 894). Boolean false
is now normalised to an empty date at the source, the canonical date and its basis travel under dedicated
authority fields, and both the direct audit and `/api/lost` reconcile to 670.
