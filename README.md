...
| `OPENAI_ADS_API_KEY`             | For ChatGPT Ads | Server-only Ads Manager key scoped to one ad account.                                                     |
| `OPENAI_ADS_API_KEYS`            | Multi-account   | Comma/newline-separated keys or a JSON string array; one account-scoped key per account.                  |
| `OPENAI_ADS_START_DATE`          | No              | Historical start date; defaults to Jan 1 of the current year.                                             |
| `OPENAI_ADS_END_DATE`            | No              | Optional fixed report end date; defaults to today.                                                        |
| `OPENAI_ADS_PIXEL_ID`            | Conversions     | Pixel ID from Ads Manager > Conversions. Not treated as an authentication secret.                         |
| `OPENAI_ADS_CONVERSIONS_API_KEY` | Conversions     | Server-only Conversions API key. No event sender is invoked automatically.                                |
...
See [ChatGPT Ads integration](docs/chatgpt-ads-integration.md) for the data flow,
supported metrics, attribution contract, conversion-event design, and required
Ads Manager setup.
...
| `GET /api/course-lead-alerts`                  | Latest complete-day leads/CPL for currently spending courses, selected-period daily baseline, 28-day trend and material-change alerts                        |
...
the latest complete day. Its expected daily leads are the course's total leads in the
selected date window divided by the window's calendar days, matching the course totals
already visible on the page. Calls without a UI date filter, including the scheduled
workflow, use month-to-date. Historical/no-current-spend rows remain available in the
explicit all-courses view, but never generate alerts. The material rules are a lead
drop, a CPL increase of at least 30% with enough volume, or at least $20 spend with zero
leads. If the shared Ads/CRM date is more than two days old, alerts are paused rather
