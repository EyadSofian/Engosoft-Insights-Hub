...
export function normalizeOpenAICreative(
  ...
  const imageUrl = safeHttpUrl(ad.creative?.image_url);
  const creativeType = ad.creative?.type?.trim() ?? "";
  return {
    platform: "chatgpt",
    account: account.name?.trim() || account.id,
    accountId: account.id,
    ...
    adId: ad.id,
    creativeId: ad.creative?.file_id?.trim() || ad.id,
    creativeName: ad.name?.trim() || `ChatGPT Ad ${ad.id}`,
    creativeType,
    mediaType: creativeType || (imageUrl ? "image" : "text"),
...
function storedCreative(row: DashboardRow): AdCreative | null {
  ...
  return {
    ...
    ad: rawString(row, "ad"),
    adId,
    creativeId: rawString(row, "creativeId") || rawString(row, "fileId") || adId,
    creativeName: rawString(row, "creativeName") || rawString(row, "ad"),
    creativeType: rawString(row, "creativeType"),
    mediaType: rawString(row, "mediaType") || rawString(row, "creativeType"),
    ...
    thumbnailUrl: safeHttpUrl(rawString(row, "thumbnailUrl") || rawString(row, "imageUrl")),
    videoUrl: safeHttpUrl(rawString(row, "videoUrl")),
    videoId: rawString(row, "videoId"),
    permalinkUrl: safeHttpUrl(rawString(row, "permalinkUrl")),
