 ...
 *
 * No sync job, closed-loop refresh or dataset writer touches them (those
 * truncate `dashboard_rows` datasets and the closed-loop graph tables only),
...
export function ensureLeadQaSchema(): Promise<void> {
  schemaReady ??= getPool()
    .query(
      `CREATE TABLE IF NOT EXISTS lead_quality_verifications (
...
export function rowToVerification(row: Row): LeadQaVerification {
  ...
  return {
    ...
    evidenceSnapshot:
      snapshot && typeof snapshot === "object" && !Array.isArray(snapshot) ? (snapshot as Record<string, unknown>) : {},
...
/** Every verdict saved for leads owned (at verification time) by one of these people. */
export async function readLeadQaVerificationsForAll(): Promise<Map<string, LeadQaVerification>> {
  await ensureLeadQaSchema();
  const result = await getPool().query<Row>(
    `SELECT ${COLUMNS} FROM lead_quality_verifications WHERE verification_status <> 'unverified'`,
  );
  return new Map(result.rows.map((row) => [String(row.crm_lead_id), rowToVerification(row)]));
}
